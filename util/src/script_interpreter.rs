...
impl ScriptInterpreter {
    ...
    pub fn new<P: AsRef<Path>>(script_path: P) -> Result<Self, Error> {
        ...
        if !path.exists() {
            return Err(
                Error::ScriptNotFound(path.to_str().unwrap().to_string()));
        }

        // Load the script into a string
        let script = match fs::read_to_string(script_path) {
            Ok(s) => s,
            Err(e) => return Err(Error::ScriptLoadError(e))
        };
        ...
        for cap in re.captures_iter(&script) {
            ...
            let exec_time_s: f64 = match cap.get(1).unwrap().as_str().parse() {
                Ok(t) => t,
                Err(e) => return Err(Error::InvalidTimestamp(format!("{}", e)))
            };
            ...
            {
                Ok(c) => c,
                Err(e) => return Err(Error::InvalidTc(
                    exec_time_s, e
                ))
            };
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Error::ScriptNotFound(s) => write!(f,
                "Could not find script file \"{}\"", s),
            Error::ScriptLoadError(e) => write!(f,
                "Could not load script: {}", e),
            Error::InvalidTimestamp(s) => write!(f,
                "Couldn't parse timestamp (\"{}\")", s),
            Error::InvalidTc(t, e) => write!(f,
                "Invalid TC at {:.02}s: {:#?}", t, e)
        }
    }
}