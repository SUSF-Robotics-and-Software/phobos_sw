...
use comms_if::{
    ...
    tc::{loco_ctrl::MnvrCmd, Tc, TcResponse},
};
...
fn main() -> Result<()> {
    ...
    // Bind the server
    let socket = MonitoredSocket::new(&ctx, zmq::REQ, socket_options, "tcp://*:5020")
    ...
    loop {
        ...
        match readline {
            ...
            Ok(line) => {
                ...
                // Serialize the TC
                let tc_str = serde_json::to_string(&tc).wrap_err("Failed to serialize the TC")?;
